...
export class PropertyFormComponent implements OnInit, OnDestroy {
  ...
  public onCancel() {
    this._customAuthService.navigateTo("properties");
  }
  ...
  public onSubmit(propertyForm: NgForm) {
    ...
    if (propertyDetails.id == -1) {
      this._createPropertySubscription = this._propertyService
      ...
      this._updatePropertySubscription = this._propertyService
        .updateProperty(propertyDetails)
        .subscribe(
          (message) =>
            this._toastService.onSuccessCall(
              "Successfully updated a property",
              propertyForm,
              "properties"
            ),
          (error) => this._toastService.onErrorCall(error)
  ...
  private getSelectedProperty() {
    this._selectedPropertySubscription = this._propertyService.selectedPropertySubject.subscribe(
      ...
      (error) =>
        this._toastService.onErrorCall(
          error,
          "Selected property fetching error"
        )
    );
  }

  private getSuburbs() {
    this._suburbsSubscription = this._suburbService
      .getSuburbs(this.query)
      .subscribe(
        ...
        (error) =>
          this._toastService.onErrorCall(error, "Suburbs fetching error")
      );
  }

  private getOwners() {
    this._ownersSubscription = this._ownerService
      .getOwners(this.query)
      .subscribe(
        ...
        (error) =>
          this._toastService.onErrorCall(error, "Owners fetching error")
      );
  }

  private getPropertyTypes() {
    this._propertyTypesSubscription = this._propertyService
      .getPropertyTypes()
      .subscribe(
